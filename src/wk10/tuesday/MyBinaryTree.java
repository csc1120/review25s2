...
public class MyBinaryTree<E> {
    ...
    private int height(Node<E> localRoot, int current, int max){
        if(localRoot == null){
            return max;
        } else {
            //??
            ...
            max = Math.max(max, height(localRoot.left, current + 1, max));
            max = Math.max(max, height(localRoot.right, current + 1, max));
            return max;
        }
        }
